...
export default function CvChecker(){
 ...
 async function upload(file?:File){if(!file)return;setFileName(file.name);setStatus("Reading your CV locally…");try{const text=await extractFile(file);setCv(text);setStatus(text.trim()?"CV ready to check":"No readable text found. Try pasting the CV text.");if(text.trim())track("cv_upload",{file_type:file.name.split(".").pop()?.toLowerCase()||"unknown"})}catch{setStatus("This file could not be read. Try another file or paste the CV text.")}}
 return <section className="checker-section" id="cv-checker"><div className="wrap">
  ...
  <div className="checker-shell"><div className="checker-inputs">
   ...
   <label><span>Paste the job description</span><textarea value={job} onChange={e=>setJob(e.target.value)} onBlur={()=>{if(job.trim().length>30)track("job_match_check",{keyword_count:analysis.matched.length+analysis.missing.length,match_score:analysis.match})}} placeholder="Paste the role requirements here to compare important keywords…"/></label>
   ...
   </>}<a className="btn btn-dark checker-cta" href="/builder" onClick={()=>track("builder_open",{source:"cv_checker"})}>Improve it in the CV builder ?</a>
  </div></div>
 </div></section>
}
